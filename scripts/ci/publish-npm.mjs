 ...
 * - Contract: file=publish-npm.yml env=NPM_PUBLISH repo=voml/von-language
 *
 * Prereq: @game-gpt/von built (dist/) before publish; CI workflow runs build.
 */
...
function publishJs(version) {
    ...
    for (const spec of JS_PACKAGES) {
        ...
        const stage = path.join(os.tmpdir(), `von-pub-js-${name.replace(/[/@]/g, '-')}-${version}`);
        ...
        if (!pkg.repository) {
            pkg.repository = {
                type: 'git',
                url: 'git+https://github.com/voml/von-language.git',
        ...
        if (outcome === 'published') published += 1;
            ...
            fail(`OIDC/auth failed for ${name}. Add Trusted Publisher: file=publish-npm.yml env=NPM_PUBLISH repo=voml/von-language`);
