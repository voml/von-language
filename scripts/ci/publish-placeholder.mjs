...
/** @type {StubSpec[]} */
const STUBS = JS_STUBS.map((name) => ({ name }));
...
function runNpm(args, opts = {}) {
    ...
    let userConfig;
    if (token) {
        userConfig = path.join(os.tmpdir(), `von-placeholder-npmrc-${process.pid}`);
...
function writeStub(spec, dir, authToken) {
    ...
    const pkg = {
        name: spec.name,
        version: VERSION,
        description: spec.description ?? 'VON placeholder — not for production use.',
        license: 'MIT',
        private: false,
        files: ['README.md'],
    };
    ...
    fs.writeFileSync(path.join(dir, 'README.md'), `# ${spec.name}\n\nPlaceholder package (${VERSION}). Reserved for the VON project.\n`);
    if (authToken) {
...
function cmdPublish() {
    ...
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'von-npm-stubs-'));
